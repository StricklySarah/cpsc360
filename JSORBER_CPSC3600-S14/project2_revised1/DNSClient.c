...
void parseName(uint8_t* buff, uint8_t* startRead, uint8_t* readInto, int startWrite){
    ...
    if(bit1 == 1 && bit2 == 1){ // pointer found
        ...
        for(i = -1; i < byte1; i++){
            memcpy(readInto+startWrite, startRead+startReadOffset+sizeof(uint8_t), sizeof(uint8_t));///uint6 or uint16
...
//send to port
    ...
    if(setsockopt(socketfd, SOL_SOCKET, SO_RCVTIMEO,(struct timeval *)&tv,sizeof(struct timeval)) == -1){
	if(flag < retries){
		 if ((sendto(socketfd, buff, qlength, 0, (struct sockaddr *)&sendaddr, sizeof(sendaddr))) < 0){
        	   printf("ERROR   sendto error\n");
     		   return 0;
